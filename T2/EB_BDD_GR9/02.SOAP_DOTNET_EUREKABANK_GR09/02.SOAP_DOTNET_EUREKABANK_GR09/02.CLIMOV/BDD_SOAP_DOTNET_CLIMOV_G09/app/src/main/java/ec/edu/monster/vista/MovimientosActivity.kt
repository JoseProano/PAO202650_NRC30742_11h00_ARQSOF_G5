...
class MovimientosActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnBuscar.setOnClickListener {
            ...
            // Primero probar con TraerSaldoCuenta para verificar que la comunicación funciona
            soapService.traerSaldoCuenta(cuenta, object : EurekaSoapDotNetService.SoapCallback<Double> {
                override fun onSuccess(saldo: Double) {
                    // Si el saldo funciona, entonces buscar movimientos
                    soapService.leerMovimientos(cuenta, object : EurekaSoapDotNetService.SoapCallback<List<Movimiento>> {
                        override fun onSuccess(result: List<Movimiento>) {
                            runOnUiThread {
                                adapter.updateMovimientos(result)
                                btnBuscar.isEnabled = true
                                btnBuscar.text = "Buscar Movimientos"
                                if (result.isEmpty()) {
                                    Toast.makeText(this@MovimientosActivity, "No se encontraron movimientos para la cuenta $cuenta (Saldo: $$saldo)", Toast.LENGTH_LONG).show()
                                } else {
                                    Toast.makeText(this@MovimientosActivity, "Se encontraron ${result.size} movimientos", Toast.LENGTH_SHORT).show()
                                }
                            }
                        }
                        ...
                        override fun onError(error: String) {
                            runOnUiThread {
                                Toast.makeText(this@MovimientosActivity, "Error al obtener movimientos: $error", Toast.LENGTH_LONG).show()
                                btnBuscar.isEnabled = true
                                btnBuscar.text = "Buscar Movimientos"
                            }
                        }
                ...
                override fun onError(error: String) {
                    runOnUiThread {
                        Toast.makeText(this@MovimientosActivity, "Error al verificar cuenta: $error", Toast.LENGTH_LONG).show()
                        btnBuscar.isEnabled = true
                        btnBuscar.text = "Buscar Movimientos"
                    }
                }
