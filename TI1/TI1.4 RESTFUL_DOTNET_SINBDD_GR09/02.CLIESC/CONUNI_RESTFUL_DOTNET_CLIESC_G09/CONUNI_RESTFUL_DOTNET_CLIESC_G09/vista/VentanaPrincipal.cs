...
namespace CONUNI_RESTFUL_DOTNET_CLIESC_G09.vista
{
    ...
    public class VentanaPrincipal : Form
    {
        ...
        private ConversionApiClient apiClient;

        public VentanaPrincipal(ConversionApiClient cliente)
        {
            apiClient = cliente;
        ...
        private void InicializarComponentes()
        {
            ...
            panelTemperatura = new PanelTemperatura(apiClient);
            panelLongitud = new PanelLongitud(apiClient);
            panelPeso = new PanelPeso(apiClient);
            panelVolumen = new PanelVolumen(apiClient);
            panelArea = new PanelArea(apiClient);
        }
        ...
        private void CerrarSesion()
        {
                ...
                MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                ventanaLogin.Show();
                // La VentanaPrincipal se cerrará cuando el login se cierre (si no hay login exitoso)
                // o cuando se cierre la nueva VentanaPrincipal creada por el login
            }
        }
