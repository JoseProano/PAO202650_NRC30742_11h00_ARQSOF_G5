...
import ec.edu.monster.databinding.FragmentVolumenBinding
...
/**
 * Fragmento para conversiones de volumen usando API REST
 */
class VolumenFragment : Fragment() {

    private var _binding: FragmentVolumenBinding? = null
    private val binding get() = _binding!!
    
    private val operaciones = listOf(
        Pair("litros", "galones") to "L → gal",
        Pair("galones", "litros") to "gal → L",
        Pair("mililitros", "onzasFluidas") to "mL → fl oz",
        Pair("onzasFluidas", "mililitros") to "fl oz → mL"
    )
    ...
    private fun setupListeners() {
        ...
        binding.btnConvertir.setOnClickListener { convertirVolumen() }
        binding.btnLimpiar.setOnClickListener { limpiarCampos() }
    }
    
    private fun convertirVolumen() {
    ...
    private fun mostrarResultado(valorOriginal: Double, origen: String, destino: String, resultado: Double) {
        val simbolos = mapOf("litros" to "L", "galones" to "gal", "mililitros" to "mL", "onzasFluidas" to "fl oz")
        binding.textResultado.text = "$valorOriginal ${simbolos[origen] ?: origen} = ${String.format("%.4f", resultado)} ${simbolos[destino] ?: destino}"
        binding.textResultado.setTextColor(resources.getColor(R.color.monster_purple, null))
