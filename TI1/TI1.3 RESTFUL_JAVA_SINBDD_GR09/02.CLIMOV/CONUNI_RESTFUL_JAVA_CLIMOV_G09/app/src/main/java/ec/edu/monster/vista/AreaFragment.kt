...
/**
 * Fragmento para conversiones de área usando API REST
 */
class AreaFragment : Fragment() {
    ...
    private val operaciones = listOf(
        Pair("metrosCuadrados", "piesCuadrados") to "m² → ft²",
        Pair("piesCuadrados", "metrosCuadrados") to "ft² → m²",
        Pair("hectareas", "acres") to "ha → ac",
        Pair("acres", "hectareas") to "ac → ha"
    )
    ...
    private fun mostrarResultado(valorOriginal: Double, origen: String, destino: String, resultado: Double) {
        val simbolos = mapOf("metrosCuadrados" to "m²", "piesCuadrados" to "ft²", "hectareas" to "ha", "acres" to "ac")
        binding.textResultado.text = "$valorOriginal ${simbolos[origen] ?: origen} = ${String.format("%.4f", resultado)} ${simbolos[destino] ?: destino}"
        binding.textResultado.setTextColor(resources.getColor(R.color.monster_light_blue, null))
